...
def _text_or_none(value: Any, max_len: int | None = None) -> str | None:
    
    if value is None:
        return None
    if isinstance(value, dict):
        for key in ("action", "recommended_action", "description", "task", "title", "summary", "root_cause", "cause", "actual_fault", "text"):
            if value.get(key):
                value = value.get(key)
                break
        else:
        ...
        if all(not isinstance(x, (dict, list, tuple, set)) for x in value):
            value = "; ".join(str(x) for x in value)
        else:
            value = _json_dumps(list(value))
...
class SteelPilotRuntimeStore:
    ...
    def save_logbook_event(self, event: dict[str, Any]) -> dict[str, Any]:
        ...
        record = {
            ...
            "updated_at": _text_or_none(now),
            "alarm_id": _text_or_none(event.get("alarm_id")),
            "row_index": row_index,
            "asset": _text_or_none(event.get("asset") or event.get("asset_name")),
            "fault": _text_or_none(event.get("fault") or event.get("predicted_fault")),
            "risk_level": _text_or_none(event.get("risk_level")),
            "risk_score": _float_or_none(event.get("risk_score")),
            "anomaly_probability": _float_or_none(event.get("anomaly_probability")),
            "rul_band": _text_or_none(event.get("rul_band") or event.get("predicted_rul_band")),
            "status": _text_or_none(event.get("status") or "open"),
            "recommended_action": _text_or_none(recommended_action, max_len=4000),
            "query": _text_or_none(event.get("query"), max_len=4000),
            "final_answer": _text_or_none(event.get("final_answer"), max_len=20000),
        ...
        with self._connect() as con:
            ...
            con.commit()
        # Return a safe scalar summary plus decoded complex fields. Do not let
        # raw event values overwrite sanitized SQLite-safe columns.
        return {
            **record,
    ...
    def logbook_display_df(self, limit: int = 200) -> pd.DataFrame:
        ...
        # Force object columns to strings to avoid ArrowInvalid mixed object types.
    ...
    def save_feedback(self, feedback: dict[str, Any]) -> dict[str, Any]:
        ...
        row_index = _int_or_none(feedback.get("row_index"))
        record = {
            "feedback_id": _text_or_none(feedback_id) or f"FDB-{uuid.uuid4().hex[:10].upper()}",
            "created_at": _text_or_none(feedback.get("created_at") or now),
            "updated_at": _text_or_none(now),
            "alarm_id": _text_or_none(feedback.get("alarm_id")),
            "row_index": row_index,
            "asset": _text_or_none(feedback.get("asset") or feedback.get("asset_name")),
            "predicted_fault": _text_or_none(feedback.get("predicted_fault")),
            "risk_level": _text_or_none(feedback.get("risk_level")),
            "anomaly_probability": _float_or_none(feedback.get("anomaly_probability")),
            "actual_fault": _text_or_none(feedback.get("actual_fault"), max_len=4000),
            "action_taken": _text_or_none(feedback.get("action_taken"), max_len=4000),
            "outcome": _text_or_none(feedback.get("outcome"), max_len=4000),
            "status": _text_or_none(feedback.get("status") or "feedback_recorded"),
            "engineer_name": _text_or_none(feedback.get("engineer_name")),
            "notes": _text_or_none(feedback.get("notes"), max_len=4000),
            "source": _text_or_none(feedback.get("source") or "streamlit"),
        ...
        with self._connect() as con:
            ...
            con.commit()
        return {**record, "raw_feedback": feedback}
