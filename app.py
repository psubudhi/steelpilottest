...
def hydrate_streamlit_secrets_to_env() -> None:
    """
    Makes Streamlit Cloud secrets available to code that uses os.getenv().
    Safe locally if no secrets are configured.
    """
    try:
...
hydrate_streamlit_secrets_to_env()
...
st.markdown(
    ...
    /* Main app text safety. Native Streamlit components mostly handle themes,
       but these rules prevent custom markdown/HTML from staying black in dark mode. */
    .stApp,
    .stApp p,
    .stApp li,
    .stApp span,
    ...
    /* Dataframe/metric related labels that sometimes inherit low-contrast colors */
...
def render_line_chart(df: pd.DataFrame, y_cols: list[str], title: str, active_ts: int | None = None) -> None:
    ...
    if alt is not None:
        ...
        chart = alt.Chart(long).mark_line(strokeWidth=2).encode(
            x=alt.X("timestamp_index:Q", title="Timestamp index"),
            y=alt.Y("value:Q", title="Value"),
            color=alt.Color("signal:N", title="Signal"),
            tooltip=["timestamp_index", "signal", "value"],
        ).properties(height=280)
        if active_ts is not None:
            rule = alt.Chart(pd.DataFrame({"timestamp_index": [active_ts]})).mark_rule(
                strokeDash=[6, 4], color="#f97316", size=2
            ).encode(x="timestamp_index:Q")
            chart = chart + rule
        chart = chart.configure_axis(
            labelColor="#2563eb",
            titleColor="#2563eb",
            gridColor="rgba(148, 163, 184, 0.28)",
            domainColor="rgba(148, 163, 184, 0.45)",
